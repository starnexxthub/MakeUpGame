    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#f2b4a9] via-[#e48f86] to-[#c63d3d] text-white px-3 sm:px-4 md:px-6 py-8 sm:py-10">
      ...
      <div className="bg-white/80 backdrop-blur-md text-red-600 rounded-xl shadow-md px-3 sm:px-4 md:px-6 py-3 sm:py-4 flex flex-wrap items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-7 md:mb-10 w-full max-w-2xl lg:max-w-4xl xl:max-w-5xl">
        <div className="flex gap-4 sm:gap-6 md:gap-8 lg:gap-10">
          ...
          <Stat label="Pairs" value={`${pairs}/10`} />
        </div>
      ...
      {/* Grid — 5×4 (20 cards) */}
      <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 gap-2 sm:gap-3 md:gap-4 lg:gap-5">
        {cards.map((card) => (
          <div
            key={card.id}
            onClick={() => handleFlip(card)}
            className={`w-14 h-14 sm:w-18 sm:h-18 md:w-22 md:h-22 lg:w-26 lg:h-26 xl:w-28 xl:h-28 rounded-xl flex items-center justify-center cursor-pointer transition-all duration-200 hover:scale-105
            ...
            style={{
              ...
              height: "clamp(3.25rem, 10vw, 7rem)",
            }}
          >
            {card.flipped || card.matched ? (
              <Image
      ...
      {showTip && currentTip && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 px-4">
          <div className="bg-[#f3d4cd] rounded-2xl shadow-xl p-4 sm:p-5 md:p-6 w-full max-w-[300px] sm:max-w-[360px] md:max-w-[420px] relative border border-red-200">
            <button
            ...
            <h2 className="text-red-500 font-semibold text-lg sm:text-xl md:text-2xl flex items-center gap-2 mb-4 sm:mb-6 md:mb-8">
              ✨ Beauty Tip!
            </h2>
